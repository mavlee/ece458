$(document).ready(function() {
  ...
  $('#data-input').keydown(function(e) {
    ...
    q = new Date();
    currentTime = q.getTime();
    currentChar = e.keyCode;
    inputTimingData[convert(currentChar)] = currentTime;
    if (previousChar != null) {
      seq = convert(previousChar) + "-" + convert(currentChar);
      if (inputData[seq] == null) {
        inputData[seq] = [];
      }
      inputData[seq].push(currentTime - previousTime);
    }
    if (previousKeydownChar != null) {
      seq = "keydown-" + convert(previousChar) + "-" + convert(currentChar);
      if (inputData[seq] == null) {
        inputData[seq] = [];
      }
      //inputData[seq].push(currentTime - previousKeydownTime);
    }
    previousChar = currentChar;
    previousTime = currentTime;
    previousKeydownChar = currentChar;
    previousKeydownTime = currentTime;
    //console.log(convert(currentChar) + " key down time: " + currentTime);
  });
  ...
  $('#data-input').keyup(function(e) {
    ...
    if (inputData[ch] == null) {
      inputData[ch] = [];
    }
    inputData[ch].push(w - inputTimingData[ch]);
  ...
  $('#test-input').keydown(function(e) {
    ...
    currentTime = q.getTime();
    currentChar = e.keyCode;
    testerTimingData[convert(currentChar)] = currentTime;
    if (previousChar != null) {
      seq = convert(previousChar) + "-" + convert(currentChar);
      if (testerData[seq] == null) {
        testerData[seq] = [];
      }
      testerData[seq].push(currentTime - previousTime);
    }
    if (previousKeydownChar != null) {
      seq = "keydown-" + convert(previousChar) + "-" + convert(currentChar);
      if (testerData[seq] == null) {
      ...
      //testerData[seq].push(currentTime - previousKeydownTime);
    }
    previousChar = currentChar;
    previousTime = currentTime;
    previousKeydownChar = currentChar;
  ...
  var checkCorrectnessManhattanDistance = function() {
    ...
    for (x in testerData) {
      if (calculatedData[x] != null) {
        for (var i = 0; i < testerData[x].length; i++) {
          tested += 1;
          console.log("test for " + x);
          //console.log("range " + (calculatedData[x]['average'] - calculatedData[x]['stddev']*1.5) + " - " + (calculatedData[x]['average'] + calculatedData[x]['stddev']*1.5));
          //console.log("tester time - " + testerData[x][i]);
  ...
  var checkCorrectnessStdDev = function () {
    // check how close you are
    var tested = 0;
    var total = 0;
    for (x in testerData) {
